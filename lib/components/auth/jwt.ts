...
export class AuthHttp {
  ...
  getAuthHeader(): Observable<any> {
      ...
      return new Observable<any>((obs: any) => {
        let header = {}
        header[this._config.headerName] = this._config.headerPrefix + this._config.tokenGetter()
        obs.next(header);
      });
  }
...
//not working with angular 2.0 final
...
export function provideAuthM(config = {}): any {
  return {
    provide: AuthHttp,
    useFactory: (http) => {
